...
跨站请求伪造，劫持受信任用户向服务器发送非预期请求的攻击方式。通常情况下，CSRF 攻击是攻击者借助受害者的 Cookie 骗取服务器的信任，可以在受害者毫不知情的情况下以受害者名义伪造请求发送给受攻击服务器，从而在并未授权的情况下执行在权限保护之下的操作

#### 验证码

csrf用户不知情的情况下构造网络请求。验证码会强调用户必须进行交互。

#### referer check

记录该HTTP请求的来源地址，利用referer check可以检查请求是否来自合法的源
...
攻击者利用这个问题，可以访问或修改数据，或者利用潜在的数据库漏洞进行攻击
